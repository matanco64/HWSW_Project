...
| 0x400 + 4·w | LEN[w] | 6 × 5 bit fields; 31:30 reserved | RW | 0 | length window, w = 0..287: table t occupies the fixed 48-word stride w = 48·t .. 48·t+47 (amendment 2026-09-08, §Amendments) — length of symbol s in table t is at word 48·t + ⌊s/6⌋, field s mod 6 (F7); fields beyond ALPHABET written 0; a write with wstrb ≠ 4'hF is ignored whole. DEFLATE: table 0 = lit/len lengths 0..ALPHABET−1, table 1 = 30 distance lengths at its own stride (software drops HDIST entries 30/31, which must be 0). A single stored copy satisfies the latching rule (writes while BUSY are ignored) — uArch choice. *(Row rewritten 2026-09-12 to match the 2026-09-08 amendment; the original continuous-packing text predated it and disagreed — caught by the first TB consumer at dv_bringup.)* |
