"""Unit test for fp64_mul (docs/rtl_contracts.md) against the tb/unit/fp_helpers oracle.

Exact green command (from the repo root, after ``source hw/env.sh``)::

    make -C hw/grape_pipeline sim TOPLEVEL=fp64_mul MODULE=unit.test_fp64_mul \
        VERILOG_SOURCES="rtl/fp64_pkg.sv rtl/fp64_mul.sv"

hw/common/Makefile.cocotb already puts <module>/tb on PYTHONPATH, so this file is
importable as ``unit.test_fp64_mul`` (tb/unit/__init__.py makes tb/unit a package).
VERILOG_SOURCES is passed on the command line because the Makefile's rtl/*.sv wildcard
also matches unrelated units; rtl/fp64_mul.sv ``include``s rtl/fp64_pkg.sv itself, but
...
subnormal in/out, underflow-tininess boundary products, halfway-rounding, benchmark dt),
10,000 uniform bit-pattern ops, 10,000 magnitude-window (1e-5..1e2) ops — result bits
AND flags against fp_helpers.ref_op — plus latency (exactly LATENCY = 3) and II = 1.
"""
import random
...
DIRECTED_VALUES = [
    ...
    f2b(2.0 ** -52), f2b(2.0 ** -53), f2b(2.0 ** -54),    # tiny powers of two
]

# targeted (a_bits, b_bits) products: underflow-tininess boundaries (tininess is judged
# after rounding with unbounded exponent — see fp64_pkg.sv), halfway cases, overflow edge
DIRECTED_PAIRS = [
    # product = nmin*(1 - 2^-54): unbounded rounding ties up to nmin -> NOT tiny, no UE
    (f2b((2 ** 27 - 1) * 2.0 ** -538), f2b((2 ** 27 + 1) * 2.0 ** -538)),
    # product = nmin*(1 - 1.5*2^-54): tiny after unbounded rounding, yet the delivered
    # denormalized-rounded result is nmin -> UE=1 with a minimum-normal result
    (f2b(5.0 * 2.0 ** -540), f2b(7205759403792793.0 * 2.0 ** -537)),
    # product = nmin*(1 - 2^-53): exactly representable unbounded (tiny), denormalized
    # rounding ties up to nmin -> UE=1 with a minimum-normal result
    (f2b((2 ** 53 - 1) * 2.0 ** -540), f2b(2.0 ** -535)),
    (f2b(2.0 ** -537), f2b(2.0 ** -538)),  # 2^-1075: tie to even -> +0, underflow
    (SMIN, f2b(0.5)),                      # tie to even -> +0, underflow
    (SMIN, f2b(0.75)),                     # rounds to SMIN, underflow
    (NMIN, f2b(0.5)),                      # exact subnormal 2^-1023: no flags
    (NMIN, f2b(1.0 - 2.0 ** -53)),         # halfway at nmin from above the boundary
    (NMAX, f2b(2.0)),                      # overflow
    (NMAX, ONE + 1),                       # round-up overflow
    (ONE + 1, f2b(1.5)),                   # halfway case in the normal range
    (SMAX, SMAX),                          # subnormal x subnormal -> +0, underflow
    (f2b(0.01), f2b(0.01)),                # benchmark dt squared
]


def _expected(ops):
    return [ref_op("mul", a, b) for (a, b) in ops]
...
async def _stream_check(dut, ops, gap_rng=None, gap_prob=0.0):
    ...
    for k, ((eb, ef), (gb, gf)) in enumerate(zip(exp, got)):
        if eb != gb or ef != gf:
            a_bits, b_bits = ops[k]
            mismatches.append(
                f"op[{k}] a={a_bits:016x} b={b_bits:016x}: "
...
async def test_directed(dut):
    """All-pairs specials/subnormals cross plus targeted boundary products."""
    await _init(dut)
    ops = [(x, y) for x in DIRECTED_VALUES for y in DIRECTED_VALUES]
    ops += DIRECTED_PAIRS
...
async def test_random_bit_patterns(dut):
    """10,000 uniform 64-bit bit-pattern operand pairs."""
    await _init(dut)
    rng = random.Random(SEED)
    ops = [(rng.getrandbits(64), rng.getrandbits(64)) for _ in range(N_RANDOM)]
    await _stream_check(dut, ops)
...
async def test_random_magnitude_window(dut):
    """10,000 magnitude-window (benchmark range) operand pairs."""
    await _init(dut)
    rng = random.Random(SEED + 1)
    ops = [(_window_bits(rng), _window_bits(rng)) for _ in range(N_RANDOM)]
    await _stream_check(dut, ops)
...
async def test_latency_and_ii(dut):
    ...
    exp_bits, exp_flags = ref_op("mul", a_bits, b_bits)
    dut.in_valid.value = 1
    dut.a.value = a_bits
    dut.b.value = b_bits
    ...
    burst = [(f2b(float(i)), f2b(0.5)) for i in range(1, 33)]
    await _stream_check(dut, burst)
    gaps = [(rng.getrandbits(64), rng.getrandbits(64)) for _ in range(300)]
