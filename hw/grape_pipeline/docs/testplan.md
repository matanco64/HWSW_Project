...
| F-08 NSTEPS=0 no-op (PRD-F8) | `test_corner::nsteps0` | `cg_cfg` NSTEPS=0 bin | DONE set, state words unchanged (readback == committed), CYCLES ≤ 4 | must | todo |
...
| F-30 K1 measurement (uArch §7 model: 123 nominal / 127 worst) | `test_smoke` (measure), `test_full_benchmark` (20000 steps) | `cg_counters` CYCLES-per-step derived bin {≤128, >128} | CYCLES/NSTEPS ≤ K1=128; report measured value vs model 123 (review carry-forward: mandatory after R6/S2 + scr_fwd restructure) | must | todo |
