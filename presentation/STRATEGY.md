...
Every prompt ends with a STATUS block (readback contract in `prompts/README_AGENT.md`): result, deck_url,
slide_url, position, title_as_typed, body, notes_set, tracker, gemini_used, deviations, screenshot.
`tools/presentation/check_status.py` diffs it against `deck.md`. Verification per slide: that check passes; the Drive connector reads the deck back; the title and notes must appear
