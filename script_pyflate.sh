#!/usr/bin/env bash
# HWSW final project — pyflate: setup, baseline, profiling + flame graph, optimized run, comparison.
# Run INSIDE the course QEMU VM (Ubuntu 22.04, python3.10). Stages are selectable:
#   ./script_pyflate.sh setup|baseline|profile|optimized|compare|all
set -euo pipefail
BENCH=pyflate
