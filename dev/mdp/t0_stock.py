"""T0 - stock pyperformance 1.14.0 bm_mdp, verbatim logic.

The only additions are (a) a phase-split timer around the two phases
(graph build == topoSort, which populates the successor cache, and the
value-iteration while-loop) and (b) a sweep counter that is returned
instead of discarded.  Nothing that runs inside a timed region differs
from stock.
"""
import collections
import time
        ...
        while dmax[initial_statep] - dmin[initial_statep] > tolerance:
            ...
            for sp in stateps:
                ...
                if sp[0] == 0:
                    ...
                    dmin[sp] = sum(dmin[sp2] * p
                                   for sp2, p in self.getSuccessors(sp))
                    dmax[sp] = sum(dmax[sp2] * p
                                   for sp2, p in self.getSuccessors(sp))
...
def solve(tolerance=TOLERANCE):
    """Run one full evaluate(), returning (result, sweeps, t_build, t_iter)."""
    b = Battle()
    initial_statep = b.initial()
    dmin, dmax, frozen = b.min, b.max, b.frozen

    t0 = time.perf_counter()
    stateps = topoSort([initial_statep], b.getSuccessorsList)
    ...
    while dmax[initial_statep] - dmin[initial_statep] > tolerance:
        ...
        for sp in stateps:
            if sp in frozen:
                continue
            if sp[0] == 0:
                dmin[sp] = max(dmin[sp2] for sp2 in b.getSuccessors(sp))
                dmax[sp] = max(dmax[sp2] for sp2 in b.getSuccessors(sp))
            else:
                dmin[sp] = sum(dmin[sp2] * p for sp2, p in b.getSuccessors(sp))
                dmax[sp] = sum(dmax[sp2] * p for sp2, p in b.getSuccessors(sp))
            if dmin[sp] >= dmax[sp]:
                dmax[sp] = dmin[sp] = (dmin[sp] + dmax[sp]) / 2
                frozen.add(sp)
    t2 = time.perf_counter()
    result = (dmax[initial_statep] + dmin[initial_statep]) / 2
    return result, itercount, t1 - t0, t2 - t1
